...
<div class="board-wrapper">
	<h2>● 일반 상품 수정하기</h2>
	<form action="./boardUpdateFinish" method="post" id="board_form" enctype="multipart/form-data">
	    ...
	    <div class="board-top">
	                    ...
	                    <h3>제목</h3>
	                        <select name="p_category_id" id="p_category_id" class="category">
	                            ...
	                            <c:forEach items="${categoryList }" var="list">
	                            	<c:if test="${board.p_category_id eq  list.p_category_id}">
	                                	<option value="${list.p_category_id}" selected="selected">${list.p_category_name }</option>
	                                </c:if>
	                                <c:if test="${board.p_category_id ne list.p_category_id}">
	                                	<option value="${list.p_category_id}">${list.p_category_name }</option>
	                                </c:if>
	                            </c:forEach>
	                        </select>
	                    <input type="text" name="board_title" value="${board.board_title }" class="title"/>
	                </div>
	                <div class="product-price">
	                    <h3>가격</h3>
	                    <input type="text" name="board_price" class="price" id="price" value="${board.board_price }" pattern="[0-9]+" onkeyup="inputNumberFormat(this)"/>
	                    <div class="chkbox">
	                    	<c:if test="${board.board_trade_type eq give_away }">
	                    	    <input type="checkbox" name="board_trade_type" value="give_away" class="radio" id="give_away" checked="checked"/>
	        ...
	        <div class="board-middle">
	            <c:forEach var="b" items="${img }" varStatus="status">
                <div class="img_div" id="img_div${status.count}">
                	<img src="${pageContext.request.contextPath}/resources/upload/board/${b.renamedfilename}" class="upload_img"
                		id="upload_img${status.count}" alt="${status.count-1}" />
                </div>
	        ...
	        <div class="board_end">
	            ...
	            <div class="content">
	                ...
	                &#10;자세한 사항은 공지사항을 확인해주세요!">${board.board_content }</textarea>
	            </div>
	            <div class="buttons">
	                <input type="button" value="수정" id="submit_btn"/>
...
upload_file.addEventListener("change", e => {
	...
	var imgs = document.getElementsByClassName("upload_img");
	for(var i = 1; i <= 6; i++){
		var img = document.getElementById("upload_img"+i);
		if(img != null){
			img.remove();
		}
	}
